...
```
PS C:> New-AzHDInsightStreamingMapReduceJobDefinition -Files {Files} -InputPath {InputPath} -Mapper {Mapper} -OutputPath {OutputPath} -Reducer {Reducer}
```

The New-AzHDInsightStreamingMapReduceJobDefinition cmdlet defines a Streaming MapReduce job object for use with an Azure HDInsight cluster. (autogenerated)
