...
```
PS C:> New-AzIntegrationAccountAssembly -AssemblyFilePath $localAssemblyFilePath -Name sampleAssembly -ParentName sampleIntegrationAccount -ResourceGroupName sampleResourceGroup
```

Creates a new assembly using the local file located at the file path contained in "$localAssemblyFilePath". (autogenerated)
